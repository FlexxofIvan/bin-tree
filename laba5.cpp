...
bool insert(subset_node** sn, int k) {
    if (*sn == nullptr) {
        // Создаем новый узел и присваиваем ему значение k
        *sn = new subset_node{k};
        return true;
    }
    if (k < (*sn)->key) {
        // Рекурсивно вставляем ключ в левое поддерево
        return insert(&((*sn)->left), k);
    } else if (k > (*sn)->key) {
        // Рекурсивно вставляем ключ в правое поддерево
        return insert(&((*sn)->right), k);
    } else {
        // Ключ уже существует в дереве, вставка не требуется
        return false;
    }
}

subset_node* find(subset_node* sn, int k) {
    if (sn == nullptr || sn->key == k) {
        // Вернуть указатель на текущий узел или nullptr, если не найдено
        return sn;
    }
    if (k < sn->key) {
...
unsigned int size(subset_node* sn) {
    if (sn == nullptr) {
        // Дерево пустое, возвращаем 0
        return 0;
    }
    // Рекурсивно вычисляем размер левого и правого поддеревьев
    unsigned int leftSize = size(sn->left);
    unsigned int rightSize = size(sn->right);
    // Общий размер равен сумме размеров поддеревьев плюс 1 (текущий узел)
...
void DFSHelper(subset_node* sn, int* result, int& index) {
    if (sn == nullptr) {
        return;
    }
    ...
    DFSHelper(sn->left, result, index);

    // Обходим правое поддерево
    DFSHelper(sn->right, result, index);
}
...
int* DFS(subset_node* sn) {
    ...
    int treeSize = size(sn); // Вычисляем размер дерева
    int* result = new int[treeSize]; // Создаем массив для хранения содержимого дерева
    int index = 0; // Индекс для заполнения массива

    subset_node** stack = new subset_node*[treeSize]; // Динамический массив указателей на узлы
    ...
    while (current != nullptr || top != -1) {
        while (current != nullptr) {
            stack[++top] = current; // Помещаем текущий узел в стек
            current = current->left; // Переходим к левому потомку
        }

        current = stack[top--]; // Извлекаем узел из стека
        result[index++] = current->key; // Добавляем значение текущего узла в массив

        current = current->right; // Переходим к правому потомку
    }

    delete[] stack; // Освобождаем память, выделенную для стека
    return result;
}
...
bool remove(subset_node** sn, int k) {
    ...
    if (k < (*sn)->key) {
        // Рекурсивно ищем узел для удаления в левом поддереве
        return remove(&(*sn)->left, k);
    }
    else if (k > (*sn)->key) {
        // Рекурсивно ищем узел для удаления в правом поддереве
        return remove(&(*sn)->right, k);
    }
    else {
        // Нашли узел для удаления

        // Если у узла нет потомков, то просто удаляем его
        if ((*sn)->left == nullptr && (*sn)->right == nullptr) {
            delete *sn;
            *sn = nullptr;
        }

            // Если у узла только один потомок, то заменяем узел на этот потомок
        else if ((*sn)->left == nullptr) {
            subset_node* temp = *sn;
            *sn = (*sn)->right;
            delete temp;
        }
            ...
            *sn = (*sn)->left;
            ...
            // Если у узла есть оба потомка, то находим узел с наименьшим значением в правом поддереве
...
//----------- Test 000 Insert of a sequent sequence
    ...
    finish = get_time();
    if (size(sn) != n) {
        cout << endl << "--- !!! Failed insert consistency or size measurement, wrong number of elements !!! ---"
             << endl;
        return 0;
    }
    if (height(sn) != n) {
...
//----------- Test 002 Remove of a sequent sequence
    ...
    if (height(sn) != 0) {
        cout << endl << "--- !!! Failed insert consistency or height measurement, too high for an empty tree !!! ---"
             << endl;
        return 0;
    }
